...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Supabase usually sends an email confirmation. We'll show a message or proceed to OTP.
      setStep(2);
    }
  };
          ...
          <p className="mt-2 text-center text-sm text-gray-400">
            {step === 1 ? (
              <>
                ...
                <Link href="/login" className="font-medium text-white hover:text-gray-300 transition-colors border-b border-white/30 hover:border-white pb-0.5">
                  Sign in
                </Link>
              </>
            ) : (
          ...
          <form className="mt-8 space-y-6" onSubmit={handleVerifyOtp}>
            <div className="space-y-4">
              <input
                ...
                className="appearance-none text-center tracking-[0.5em] text-2xl relative block w-full px-4 py-4 border border-white/20 bg-transparent placeholder-gray-500 text-white rounded-lg focus:outline-none focus:ring-1 focus:ring-white focus:border-white focus:z-10 transition-all"
